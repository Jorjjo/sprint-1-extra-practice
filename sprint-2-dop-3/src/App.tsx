...
export const App = () => {
    ...
    const removeFTRoute = (flightTableID: string, routeID: string) => {
        setFlightTables(
            flightTables.map((flightTable) =>
                flightTable.flightTableID === flightTableID
                    ? {
                          ...flightTable,
                          routes: flightTable.routes.filter(
    ...
    const addNewFT = (from: string, to: string) => {
        const newFlightTable = {
            ...
            flightTableID: v1(),
            routes: [{ id: v1(), from, to, isBooked: false }],
        };
    ...
    const removeFT = (flightTableID: string) => {
        setFlightTables(
            flightTables.filter((ft) => ft.flightTableID !== flightTableID),
        );
    };
    ...
    return (
        <>
            <header className={styles.header}>
                <AddNewItem title={'Add New FlightTable'} onClick={addNewFT} />
            </header>
